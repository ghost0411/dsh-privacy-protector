 ...
 */
function readRegistry(ctx: unknown): unknown {
  const holder = ctx as { deepseekLlmApiExtensions?: unknown; get?: (name: string) => unknown }
  try {
 ...
 */
export function probeSessionLogUpload(ctx: unknown): SessionLogUploadProbe {
  ...
  const providers = (registry as { providers?: unknown }).providers
