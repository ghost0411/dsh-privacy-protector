...
/** Mirrors `SessionTelemetryRecord` from `@deepseek-ai/dsh-session-telemetry`. */
...
function redactValue(
  ...
  if (depth > MAX_DEPTH || budget.nodes <= 0) return value
 ...
 */
export function redactRecord(record: SessionTelemetryRecord, rules: Rule[]): SessionTelemetryRecord {
  ...
  if (attributes === record.attributes && body === record.body) return record
  return {
    ...record,
 ...
 */
export function installTelemetryRedaction(ctx: Context, rules: Rule[]): void {
