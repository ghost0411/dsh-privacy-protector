...
function writeJsonAtomic(file: string, obj: unknown): void {
  ...
  try {
    ...
    try { writeFileSync(file, JSON.stringify(obj), 'utf8') } catch { /* last resort */ }
    ...
    setImmediate(() => { try { renameSync(tmp, file) } catch { try { renameSync(tmp, file) } catch { /* give up */ } } })
    void cause
  }
}
...
function readJson(file: string): { parsed: unknown } | null {
  try {
    const raw = readFileSync(file, 'utf8')
    if (raw.trim().length === 0) return { parsed: null }
...
// ---------------------------------------------------------------------------
// Toggle store (plain JSON, no secrets)
// ---------------------------------------------------------------------------
...
export function createToggleFileStore(dir: string): ToggleFileStore {
  ...
  return {
    load() {
      ...
      const sessionsRaw = typeof o.sessions === 'object' && o.sessions !== null ? (o.sessions as Record<string, unknown>) : {}
      const sessions: Record<string, boolean> = {}
      for (const [k, v] of Object.entries(sessionsRaw)) sessions[k] = v === true
      const guardianRaw = typeof o.guardian === 'object' && o.guardian !== null ? (o.guardian as Record<string, unknown>) : null
      let guardian: GuardianSnapshot | undefined
      if (guardianRaw) {
        const armed = guardianRaw.armed === true
        const gSessionsRaw = typeof guardianRaw.sessions === 'object' && guardianRaw.sessions !== null ? (guardianRaw.sessions as Record<string, unknown>) : {}
        const gSessions: GuardianSnapshot['sessions'] = {}
        for (const [k, v] of Object.entries(gSessionsRaw)) {
          if (typeof v !== 'object' || v === null || Array.isArray(v)) continue
          const clean: Partial<Record<TopicCategory, number>> = {}
          for (const [cat, n] of Object.entries(v as Record<string, unknown>)) {
            if (typeof n === 'number' && Number.isSafeInteger(n) && n > 0) clean[cat as TopicCategory] = n
          }
          if (Object.keys(clean).length > 0) gSessions[k] = clean
        }
...
// ---------------------------------------------------------------------------
// Vault store (encrypted; PII never in cleartext)
// ---------------------------------------------------------------------------
 ...
 */
export function detectSafeStorage(): Encryptor | null {
  try {
    const require = createRequire(import.meta.url) as (id: string) => Record<string, unknown>
    const electron = require('electron') as { safeStorage?: { isEncryptionAvailable?: () => unknown; encryptString?: (t: string) => Buffer; decryptString?: (b: Buffer) => string } }
    const backend = electron?.safeStorage
    if (!backend || typeof backend.encryptString !== 'function' || typeof backend.decryptString !== 'function') return null
    const available = backend.isEncryptionAvailable?.()
    if (available instanceof Promise) return null
...
export function createVaultFileStore(dir: string, encryptor: Encryptor | null): VaultFileStore {
  ...
  if (!encryptor) {
    return {
      ...
      load() { return null },
      save() { /* no-op: never write PII in cleartext */ },
    }
  }
  return {
    persistent: true,
    ...
    load() {
      ...
      if (typeof obj !== 'object' || obj === null || Array.isArray(obj)) return null
      const o = obj as Record<string, unknown>
      if (o.alg !== 'safeStorage' || typeof o.data !== 'string') return null
      let text: string
      try {
      ...
      const countersRaw = typeof p.counters === 'object' && p.counters !== null ? (p.counters as Record<string, unknown>) : {}
      const counters: Record<string, number> = {}
      for (const [k, v] of Object.entries(countersRaw)) if (typeof v === 'number' && Number.isSafeInteger(v)) counters[k] = v
