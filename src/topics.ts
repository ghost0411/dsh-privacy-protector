 ...
 */
const SELF_REF =
  '(?:我|本人|自己|我们家|我们单位|我们公司|我爸|我妈|我爸妈|我的|自己的|我手头)'
...
export const TOPIC_RULES: TopicRule[] = [
  {
    category: 'FINANCE',
    pattern: new RegExp(
      `${SELF_REF}(?:每?月|年)?(?:大概|大约)?(?:工资|月薪|年薪|薪资|薪水|收入|存款|积蓄|身价|股票|基金|理财|负债|欠款|房贷|车贷|信用卡)[\\s是大概约]*${NUM}(?:万|千|块|元)?`,
    ),
  },
  {
    category: 'FINANCE',
    ...
    pattern: new RegExp(
      `${SELF_REF}(?:大概|大约)?(?:(?:每|一)?个?月|年)?(?:挣|赚|入账|到手|工资到手|收入)[\\s是大概约]*${NUM}(?:万|千|k|w|块|元)?`,
    ),
  },
  {
    category: 'HEALTH',
    pattern: new RegExp(
      `${SELF_REF}(?:患有|得了|确诊了?|查出(?:过)?|诊断出|有)?(?:糖尿病|高血压|心脏病|乙肝|癌症|肿瘤|抑郁症|焦虑症|失眠|肾病|尿毒症|白血病|化疗)`,
    ),
  },
  {
    category: 'HEALTH',
    ...
    pattern: new RegExp(
      `(?:[，。；,])(?:还)?(?:有|得了|患了|确诊了?|查出)(?:糖尿病|高血压|心脏病|乙肝|癌症|肿瘤|抑郁症|焦虑症|失眠|肾病|尿毒症|白血病|化疗)`,
    ),
  },
  {
    category: 'HEALTH',
    pattern: new RegExp(
      `${SELF_REF}每?(?:天|日)?(?:在?吃|在?服用|需要打|在?(?:吃)??)、(?:降压药|降糖药|安眠药|止痛药|抗抑郁药|胰岛素|化疗药|避孕药)`,
    ),
  },
  {
    category: 'WORK',
    pattern: new RegExp(
      `${SELF_REF}在.+?(?:公司|单位|部门|团队|厂子)做(.{1,20}?)`,
    ),
  },
  {
    category: 'WORK',
    pattern: new RegExp(
      `${SELF_REF}(?:负责|搞|管|经手|接触|处理).{0,12}(?:核心|机密|保密|客户|报价|成本|财务收支|采购|招标|源代码|源码|配方|研发计划)`,
    ),
  },
  {
    category: 'RELATIONSHIP',
    pattern: new RegExp(
      `${SELF_REF}(?:和|跟|与)(?:我)?(?:老公|老婆|丈夫|妻子|男朋友|女朋友|男友|女友|对象|伴侣|另一半|孩子|父母|婆婆|岳母)(?:的?)(?:感情|关系|性生活|房事|坐月子|离婚|吵架|冷战|出轨|家暴|性格)`,
    ),
  },
  {
    category: 'RELATIONSHIP',
    pattern: new RegExp(
      `${SELF_REF}(?:最近|现在)?(?:和|跟|与).{0,8}(?:吵架|闹离婚|冷战|分手|离婚)`,
    ),
  },
]
...
const PLACEHOLDER = (category: TopicCategory) => `[屏蔽:${category}:1]`
 ...
 * stops.
 */
export const INDUCE_RULES: RegExp[] = [
  // "你/你的 X ... 多少/怎么样/如何/什么情况"
  new RegExp(
    `(?:你|您)(?:的)?(?:(?:一?个?月|每月|每年|现在)?的)?`
      + `(?:工资|月薪|年薪|薪资|收入|存款|工资条|银行卡|余额|负债|房贷)`
      + `[\\s是大概约]*(?:多少|多少呢|是多少|几|怎么样|怎样|如何|什么情况|多少收入)`,
  ),
  new RegExp(
    `(?:你|您)(?:的)?(?:体检|体检报告|病历|病况|病情|血压|血糖|体重|身高|吃药|用药|检查结果)`
      + `[\\s是大概约]*(?:多少|怎么|怎么样|怎样|如何|什么情况|结果)`,
  ),
  new RegExp(
    `(?:你|您)(?:的)?(?:对象|老公|老婆|丈夫|妻子|男朋友|女朋友|男友|女友|伴侣|另一半|家庭|父母)`
      + `[\\s是大概约]*(?:关系|感情|怎么样|怎样|如何|什么情况|谁|哪(?:里|位)|出轨|吵架)`,
  ),
  // "收入类的询问：...挣/赚/花多少/大概多少"
  new RegExp(
    `(?:你|您)(?:一?个?月|每年|大概|大约|现在)?(?:大概|大约)?(?:能)?(?:挣|赚|花|开销|收入|拿)(?:多少|几|[\\s]*(?:k|w|块|元))`,
  ),
  // "告诉/说说/分享/透露/聊聊 ... (你/你的) X"
  new RegExp(
    `(?:告诉|说说|分享|透露|聊聊|聊一下|介绍一下|讲讲)(?:我|我们)?[\\s，,]*`
 ...
 * passthrough.
 */
export function scanTopics(text: string): TopicScanResult {
  ...
  if (hits.length === 0) return { categories: [], masked: text, hit: false }

  hits.sort((a, b) => (a.start !== b.start ? a.start - b.start : b.end - a.end))
  const selected: TopicHit[] = []
  let lastEnd = -1
  ...
  const seen = new Set<TopicCategory>()
 ...
 */
export function hasInducement(text: string): boolean {
