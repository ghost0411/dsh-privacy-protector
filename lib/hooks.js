...
function latestRoleTexts(messages) {
    let assistant = '';
    let user = '';
 ...
 */
function isActedInducement(assistantText, userText) {
...
export function registerHooks(ctx, vault, rules, isEnabled, log, guardian, onFirstUse) {
    ...
    ctx.on('tools/pre-execute', async (exec, next) => {
        if (isEnabled(sessionIdOf(exec))) {
            ...
            if (leaked > 0) {
                await log(`warning: ${leaked} placeholder(s) reached tool "${String(exec.name ?? '?')}" un-restored; ` +
 ...
 */
function splitPartialToken(text) {
    const open = text.lastIndexOf('[');
    if (open < 0)
        return [text, ''];
    const suffix = text.slice(open);
    if (!PARTIAL_TOKEN_RE.test(suffix))
        return [text, ''];
    return [text.slice(0, open), suffix];
}
function appendFragment(tails, kind, index, text, vault, identity) {
    const key = `${kind}:${index}`;
    const combined = (tails.get(key)?.text ?? '') + text;
    const [head, tail] = splitPartialToken(combined);
    if (tail)
        tails.set(key, { kind, index, text: tail, ...identity });
    else
        tails.delete(key);
