 ...
 */
export function createToggleRegistry(onMutate) {
    ...
    const notify = () => {
        ...
        catch { /* persistence must never break the toggle */ }
    };
    return {
        isEnabled(sessionId) {
            if (global)
        ...
        snapshot() {
            const sessions = {};
            for (const [id, enabled] of bySession)
                sessions[id] = enabled;
